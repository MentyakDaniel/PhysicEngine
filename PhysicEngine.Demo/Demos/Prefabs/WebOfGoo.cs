    ...
    public class WebOfGoo
    {
        ...
        public WebOfGoo(World world, Vector2 position, float radius, int rings, int sides)
        {
            ...
            for (int i = 1; i < rings; i++)
            {
                ...
                JoinHelper.LinearStiffness(4.0f, 0.5f, jointClose.BodyA, jointClose.BodyB, out var stiffness, out var damping);
            ...
            for (int i = 0; i < _ringBodys[rings - 2].Count; i++)
            {
                ...
                JoinHelper.LinearStiffness(8.0f, 0.5f, joint.BodyA, joint.BodyB, out var stiffness, out var damping);
            ...
            for (int i = 1; i < _ringBodys.Count; i++)
            {
                for (int j = 0; j < sides; j++)
                {
                    ...
                    JoinHelper.LinearStiffness(4.0f, 0.5f, joint.BodyA, joint.BodyB, out var stiffness, out var damping);
                    joint.Stiffness = stiffness;
                    joint.Damping = damping;
                    joint.Breakpoint = Breakpoint;
                    world.AddJoint(joint);
